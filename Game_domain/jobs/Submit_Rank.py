...
"""init2_df method"""
# sorting two dataframe with same name with exp point
# 1) 
# 같은 이름이 두 데이터 프레임에 존재하지 않는다면 (3만등 아래로 떨어지거나/ 혹은 전날대비 새로 진입했다면)
# 해당 인스턴스는 무시한다.
# 2)
# 경험치 상승의 절대적인 량을 조사하는데, 레벨이 달라졌다면 무시한다.
# 레벨이 오른 경우는 따로 컬럼을 생성한다. --> 레벨이 달라진 row 중 기존 레벨이 높은 순서대로 정렬 (전날 경험치도 함께?)


# set a key value --> 데이터 시각화 키값 기준
